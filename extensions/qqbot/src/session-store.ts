 ...
 * 支持进程重启后通过 RESUME 机制快速恢复连接（5 分钟窗口内、appId 匹配时）。
 *
 * 移植自上游 @tencent-connect/openclaw-qqbot session-store.ts：
 * - import getQQBotDataDir 改为 fork 的 ./platform.js（消除对上游 utils/platform.js 的依赖）
 * - 日志改用 createLogger("qqbot:session")（替换裸 console.log/error）
 * Design doc §4.2 (P0-E)。
 */
...
const logger = createLogger("qqbot:session");
...
// Session 文件目录（~/.openclaw/qqbot/sessions）
...
export function loadSession(accountId: string, expectedAppId?: string): SessionState | null {
  ...
  try {
    ...
    if (now - state.savedAt > SESSION_EXPIRE_TIME) {
      logger.info(
        `Session expired for ${accountId}, age: ${Math.round((now - state.savedAt) / 1000)}s`,
      );
      try {
    ...
    if (expectedAppId && state.appId && state.appId !== expectedAppId) {
      logger.info(
        `appId mismatch for ${accountId}: saved=${state.appId}, current=${expectedAppId}. Discarding stale session.`,
      );
      try {
        fs.unlinkSync(filePath);
      } catch {
    ...
    if (!state.sessionId || state.lastSeq === null || state.lastSeq === undefined) {
      logger.info(`Invalid session data for ${accountId}`);
      return null;
    }

    logger.info(
      `Loaded session for ${accountId}: sessionId=${state.sessionId}, lastSeq=${state.lastSeq}, appId=${state.appId ?? "unknown"}, age=${Math.round((now - state.savedAt) / 1000)}s`,
    );
    return state;
  } catch (err) {
...
function doSaveSession(state: SessionState): void {
  ...
  try {
    ...
    logger.info(
      `Saved session for ${state.accountId}: sessionId=${state.sessionId}, lastSeq=${state.lastSeq}`,
    );
  } catch (err) {
    logger.error(`Failed to save session for ${state.accountId}: ${err}`);
  }
}
...
export function clearSession(accountId: string): void {
  ...
  try {
    if (fs.existsSync(filePath)) {
      fs.unlinkSync(filePath);
      logger.info(`Cleared session for ${accountId}`);
    }
  } catch (err) {
    logger.error(`Failed to clear session for ${accountId}: ${err}`);
  }
}
      ...
      if (file.startsWith("session-") && file.endsWith(".json")) {
        ...
        try {
          ...
          if (now - state.savedAt > SESSION_EXPIRE_TIME) {
            fs.unlinkSync(filePath);
            cleaned++;
            logger.info(`Cleaned expired session: ${file}`);
          }
        } catch {
          // 忽略解析错误，但也删除损坏的文件
          try {
